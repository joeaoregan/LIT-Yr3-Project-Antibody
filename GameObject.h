...
public:
	...
	// Timers
	unsigned int getLastTime() const { return m_LastTime; }				// Get the previous time for counter
	...
	int getNumRockets() const { return m_NumRockets; }					// Get the number of rockets in a players inventory
	// Weapons
	int getPlayer() const { return m_Player; }							// Check if a weapon is assigned to player 1 or 2
	int getDamage() const { return m_Damage; }							// Damage an object inflicts
	bool getKillRocket() const { return m_KillRocket; }					// Is a rocket to be destroyed
	int getBonusScore() const { return m_BonusScore; }					// Get bonus score for rockets
	// Shield
	bool getShieldActive() const { return m_ShieldActive; }				// Is the shield active or not
	bool getSawActive() const { return m_SawActive; }					// Is the Players saw active
	// Laser
	int getLaserGrade() const { return m_LaserGrade; }					// Get the Players laser grade
	// Speed boost
	bool getSpeedBoost() const { return m_SpeedBoost; }					// Is the Players speed boost active
	unsigned int getBoostStartTime() const { return m_BoostStartTime; }	// Get the Players speed boost start time
	float getBoostPercent() const { return m_BoostPercent; }			// Get the percentage for the boost status bar
	// Blood Cells
	int getDistanceBetween() const { return m_DistanceBetween; }		// Distance on X axis between spawning bloodcells
	// Rotating objects
	int getRotationDirection() const { return m_RotationDirection; }	// 2017/01/30 Added rotation direction, so Blood Cells can rotate both forwards and backwards
	...
	// Timers
	void setLastTime(unsigned int time) { m_LastTime = time; }			// Set the previous time for counter
	...
	// Collider
	void setCollider(SDL_Rect collider) { m_Collider = collider; }		// 2017/01/19 Set the objects collider
	...
	void setNumRockets(int rocket) { m_NumRockets = rocket; }				// Set the number of rockets in a players inventory
	// Blood Cells
	void setDistanceBetween(int d) { m_DistanceBetween = d; }			// Set distance between spawning blood cells
	// Rotating objects
	void setRotationDirection(int d) { m_RotationDirection = d; }		// 2017/02/18 Moved Blood Cells to game object list
	// Weapons
	void setPlayer(int p = 0) { m_Player = p; }							// Set the player the weapon is assigned to
	void setDamage(int d = 0) { m_Damage = d; }							// Set the damage an object inflicts
	void setKillRocket(bool kill = 0) { m_KillRocket = kill; }			// Set a rocket to be destroyed
	void setBonusScore(int bonus = 0) { m_BonusScore = bonus; }			// Set bonus score for rockets
	void setShieldActive(bool active) { m_ShieldActive = active; }		// Set the saw active
	void setSawActive(bool active) { m_SawActive = active; }			// Set the saw active
	...
	void setBoostStartTime(unsigned int time) { m_BoostStartTime = time; }// Set the players speed boost start time
	void setBoostPercent(float speed) { m_BoostPercent = speed; }		// Set the percenage for the boost status bar
...
protected:
	...
	int m_NumLives;					// The players number of lives
	...
	unsigned int m_LastTime = 0.0;	// Previous stored time
	...
	int m_DistanceBetween;			// Distance between Blood Cells spawning
	int m_RotationDirection;		// Direction to rotate Blood Cell (-1 = anticlockwise, 1 = clockwise)

	// Weapons
	int m_Player;					// The player a weapon is assigned to
	bool m_SawActive;				// Set players saw active
	bool m_ShieldActive;			// 2017/03/13 Turn on / off shield
	int m_BonusScore;				// Store the bonus score for a rocket taking out an enemy
	...
	int m_NumRockets;				// The number of rockets an object has
	bool m_KillRocket;				// Destroy the rocket
	bool m_SpeedBoost;				// Speed boost is active
	unsigned int m_BoostStartTime;	// Time the speed boost started
	float m_BoostPercent = 3.0;		// How long the speed boost lasts
	int m_LaserGrade;				// The grade of a Players laser (Single, double, or triple)

	// particles
	bool m_DrawParticle;			// Draw particles for an object or not
};
...
bool m_LineAlgorithmCalculated;	// Used in scoreText
